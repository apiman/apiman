 ...
 * sensitive data.  Should be compatible with all implementatons of
 * {@link IStorage}.  For example, this is responsible for encrypting
 * the configuration information in the {@link PolicyBean}.
 *
 * @author eric.wittmann@redhat.com
 */
public class SecureStorageWrapper implements IStorage {

    private final IStorage delegate;
    @SuppressWarnings("unused")
    private final WarApiManagerConfig config;

    /**
     * Constructor.
     * @param delegate
     */
    public SecureStorageWrapper(WarApiManagerConfig config, IStorage delegate) {
        this.delegate = delegate;
        // TODO add custom encryption support
        this.config = config;
    }
    ...
    public void createPolicy(PolicyBean policy) throws StorageException {
        // Encrypt the configuration here - there may be sensitive information in there.
        policy.setConfiguration(AesEncrypter.encrypt(policy.getConfiguration()));
    ...
    public PolicyBean getPolicy(PolicyType type, String organizationId, String entityId, String version,
        ...
        PolicyBean policy = this.delegate.getPolicy(type, organizationId, entityId, version, id);
        policy.setConfiguration(AesEncrypter.decrypt(policy.getConfiguration()));
        return policy;
    }
