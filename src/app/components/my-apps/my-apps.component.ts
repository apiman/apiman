...
import {IApiStatus, IClientStatus, statusColorMap } from '../../interfaces/IStatus';
import {
  IClientSummaryBean,
...
export class MyAppsComponent implements OnInit {
  ...
  private fetchContracts(){
    this.backend.getEditableClients().pipe(
      ...
      flatMap((clientSum: IClientSummaryBean) => {
        return this.backend.getClientVersions(clientSum.organizationId, clientSum.id);
      }),
      flatMap((versions: IClientVersionSummaryBean[]) => {
        return versions;
      }),
      flatMap((versionSum: IClientVersionSummaryBean) => {
        return this.backend.getContracts(versionSum.organizationId, versionSum.id, versionSum.version);
      }),
      flatMap((contracts: IContractSummary[]) => {
        return contracts;
      }),
      flatMap((contractSum: IContractSummary) => {
        return this.backend.getContract(contractSum.clientOrganizationId, contractSum.clientId, contractSum.clientVersion, contractSum.contractId);
      })
  ...
  private extractContracts(contracts: IContractExt[]){
    ...
    this.contracts.forEach((contract: IContractExt) => {
      const clientNameVersionMapped = contract.client.client.name + ':' + contract.client.version;
      ...
      if (foundContracts){
        ...
        this.clientContractsMap.set(clientNameVersionMapped, foundContracts.concat(contract));
      }else{
  ...
  private extendContract(contract: IContract): IContractExt{
    let contractExt = contract as IContractExt;
