...
describe.skip('Testing the api-details page', () => {
  ...
  it('Check Hero Image and Title home', () => {
    ...
    cy.get('#hero-logout-btn').should('exist').and('be.visible');
  });
  ...
  it('Check Search Input', () => {
    ...
    cy.get('#CypressTestClient1-1\\.0').should('exist');
    cy.checkTocLength(1);

    // Check search with client name
    cy.get('.search-input').type('CypressTestClient');
    cy.get('#CypressTestClient1-1\\.0').should('exist');
    cy.checkTocLength(1);
    ...
    cy.get('.search-input').type('CypressTestApi');
    cy.get('#CypressTestClient1-1\\.0').should('exist');
    cy.checkTocLength(1);
    cy.get('#search-input-clear-btn').should('exist').click();
    cy.get('.search-input').should('have.text', '');
    ...
    cy.get('#CypressTestClient1-1\\.0').should('not.exist');
    cy.checkTocLength(0);
  ...
  it('Check Client Header', () => {
    cy.get('#CypressTestClient1-1\\.0').within(() => {
      cy.get('.app-header').should('include.text', 'CypressTestClient1 - 1.0');
  ...
  it('Check Summary Section', () => {
    cy.get('#CypressTestApi1').within(() => {
      ...
      cy.get('#CypressTestClient1-1\\.0-CypressTestApi1-1\\.0')
        .should('exist')
        .and('include.text', 'CypressTestApi1 1.0 - CypressTestPlan1');
      ...
      cy.get('#api-endpoint-input').should(
        'have.value',
        'https://vagrantguest/pas/gateway/CypressTestOrg/CypressTestApi1/1.0'
      );
