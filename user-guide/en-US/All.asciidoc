...
```
<1> Please check for the latest version, this is just an example.
...
{
    "groupId": "io.apiman.plugins",
    "artifactId": "apiman-plugins-http-security-polic",
    "version": "1.2.5.Final" <1>
}
```
<1> Please check for the latest version, this is just an example.
...
| preload (Enable HSTS preload flag)
| Boolean
...
Options for `contentSecurityPolicy` are:
