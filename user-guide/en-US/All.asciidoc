...
==== Plans
A plan is a set of policies that define a level of API. Whenever an API is consumed it must be
...
like authentication.  Any policies configured on API will be applied at runtime regardless of the
client app and API contract.  This is why authentication is a common policy to configure at the API
level.

APIs must be offered through a valid plan configured in the same organization. API consumers must
consume the API through one of those plans. Please see the section on 'API Contracts' for more
information.

TIP: Note that, alternatively, APIs can be marked as "Public", which means they (once published) can be
used by any client, without needing a Contract.  To use a public API, simply send requests to the
...
published to the runtime gateway for consumption by client apps.  Once this is done, the API cannot be
changed. If changes are required, a new version of the API must be created and configured.
...
Once the API contracts are created, the client app can be registered with the runtime gateway. Once this
registration is complete, the client app can no longer be altered.  If changes are required, a new version
of the client app must be created and configured.
...
requests made to the API through the API Gateway must include this API key. The API key is used to create
the runtime policy chain from the policies configured on the API, plan, and client app.
...
provide. This section explains the steps necessary for end users to provide those APIs.
...
be required when proxying requests to the back end API.  For example, if you are using mutual/two-way
...
gateway. This can be done from the 'Overview' tab of the API details page. Simply click the 'Publish'
button on the 'Overview' tab to publish the API to the runtime gateway. If successful, the status of the
API will change to "Published" and the 'Publish' button will disappear.

It is worth repeating that the 'Publish' button will be disabled until the API is fully configured (at
which time it transitions to "Ready" status). This includes at least the implementation and one or more
available plans. API policies are optional.
...
If an API has been used by at least one client, then it will have metrics information available.
...
In order to consume an API you must first create a client app. Client Apps must exist within the
...
From the client app details page select the 'Overview' tab. Click on the 'Search for APIs to consume'
...
presented with an additional link on the consumer-oriented API details page.  This link will take you
to a page where you can browse the detailed documentation for the API.  The detailed documentation
should be very helpful in learning what resources and operations are supported by the API, which will
aid in figuring out how precisely to consume the API.
...
To register the client app with the gateway navigate to the 'Overview' tab on the client app details page.
The status of the client app should be "Ready", and the 'Register' button should be enabled. Click the
'Register' button to register the client app with the runtime gateway. If successful, the client app status
...
you must know the URL of the managed API.  This URL includes the API Key generated for the API
Contract.
...
a particular API can be different based on the Gateway implementation.  However by defaul the
endpoint format is:
